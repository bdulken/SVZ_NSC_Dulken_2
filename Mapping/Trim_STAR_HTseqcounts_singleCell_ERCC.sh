...
#Adjust trim_galore for read length
	filePath="${Dir}"
	outPath="${Dir}/TRIMMED"
	for f in $(find "$filePath" -name '*_R1_001.fastq')
	do
		base2=$(basename "${f}" | sed 's/_R1_001\.fastq/_R2_001\.fastq/g');
		f2="${Dir}/${base2}"
		
		trim_galore -q 15 --phred33 --paired --length 30 -a CTGTCTCTTATA --stringency 3 $f $f2
		mv *_val_*.fq $outPath
		mv *txt $outPath
	done
...
#2. run tophat mapping into BAM format
	filePath="${outPath}"
	for f in $(find "$filePath" -name '*_val_1.fq')
	do
		file2=$(basename "${f}" | sed 's/R1_001_val_1\.fq/R2_001_val_2\.fq/g');
		f2="${filePath}/${file2}"
	
		folderName=$(basename "${f}" | sed 's/_L001_R1_001_val_1\.fq//g');
		oPath="${Dir}/STAR"
...
#3. convert to BAM
	...
	do
		fileName=$(basename "${f}" | sed 's/_L001_R1_001_STAR\.sam/_STAR\.bam/g')
